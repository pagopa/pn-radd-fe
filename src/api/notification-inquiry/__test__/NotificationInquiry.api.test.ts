...
describe('NotificationInquiry Api', () => {
  ...
  it('getPracticesByIun ACT ok', async () => {
    mock
      .onGet(GET_PRACTICES_BY_IUN_PATH('act', 'test'))
      .reply(200, data.NOTIFICATION_INQUIRY.INQUIRY_BY_IUN_OK);
  ...
  it('getPracticesByIun AOR ok', async () => {
    mock
      .onGet(GET_PRACTICES_BY_IUN_PATH('aor', 'test'))
      .reply(200, data.NOTIFICATION_INQUIRY.INQUIRY_BY_IUN_OK);
  ...
  it('getActTransactionByOperationId ok', async () => {
    mock
      .onGet(GET_TRANSACTION_BY_OPERATION_ID_PATH('test', 'act'))
      .reply(200, data.NOTIFICATION_INQUIRY.INQUIRY_ACT_BY_OPERATION_ID_OK);
    const res = await NotificationInquiryApi.getActTransactionByOperationId('test');
    expect(res).toStrictEqual(
      NotificationInquiryConverter.operationActResponseToOperationsResponse(
  ...
  it('getAorTransactionByOperationId ok', async () => {
    mock
      .onGet(GET_TRANSACTION_BY_OPERATION_ID_PATH('test', 'aor'))
      .reply(200, data.NOTIFICATION_INQUIRY.INQUIRY_AOR_BY_OPERATION_ID_OK);
    const res = await NotificationInquiryApi.getAorTransactionByOperationId('test');
    expect(res).toStrictEqual(
      NotificationInquiryConverter.operationAorResponseToOperationsResponse(
  ...
  it('getActPracticesByInternalId ok', async () => {
    mock
      .onPost(GET_PRACTICES_BY_INTERNAL_ID_PATH('test', 'act'))
      .reply(200, data.NOTIFICATION_INQUIRY.INQUIRY_ACT_BY_INTERNAL_ID_OK);
    const res = await NotificationInquiryApi.getActPracticesByInternalId('test');
    expect(res).toStrictEqual(
      NotificationInquiryConverter.operationsActDetailsToOperationsResponse(
  ...
  it('getAorPracticesByInternalId ok', async () => {
    mock
      .onPost(GET_PRACTICES_BY_INTERNAL_ID_PATH('test', 'aor'))
      .reply(200, data.NOTIFICATION_INQUIRY.INQUIRY_AOR_BY_INTERNAL_ID_OK);
    const res = await NotificationInquiryApi.getAorPracticesByInternalId('test');
    expect(res).toStrictEqual(
      NotificationInquiryConverter.operationsAorDetailsToOperationsResponse(
