...
describe('TransactionApi', () => {
  ...
  it('startTransaction ok', async () => {
    mock
      .onPost(GET_START_TRANSACTION_PATH('act'))
      .reply(200, data.TRANSACTION.START_TRANSACTION_OK);
  ...
  it('completeTransaction ok', async () => {
    mock
      .onPost(GET_COMPLETE_TRANSACTION_PATH('act'))
      .reply(200, data.TRANSACTION.COMPLETE_TRANSACTION_OK);
  ...
  it('abortTransaction ok', async () => {
    mock
      .onPost(GET_ABORT_TRANSACTION_PATH('act'))
      .reply(200, data.TRANSACTION.ABORT_TRANSACTION_OK);
