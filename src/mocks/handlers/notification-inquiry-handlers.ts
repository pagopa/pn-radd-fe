...
import {
  PRACTICES_BY_INTERNAL_ID_PATH,
  PRACTICES_BY_IUN_PATH,
  TRANSACTION_BY_OPERATION_ID_PATH,
} from '../../api';
import {
...
export const enabledHandler = [
  rest.get(`${API_BASE_URL}${PRACTICES_BY_IUN_PATH}`, (req, res, ctx) => {
    ...
    const response =
      MAP_IUN_TO_ERROR_RESPONSE[iun as string] ?? data.NOTIFICATION_INQUIRY.INQUIRY_BY_IUN_OK;
  ...
  rest.get(`${API_BASE_URL}${TRANSACTION_BY_OPERATION_ID_PATH}`, (req, res, ctx) => {
    ...
    if (domain === 'act') {
      const MAP_OPERATION_ID_TO_ACT_ERROR_RESPONSE: { [key: string]: OperationActResponse } = {
        '67d8a1f225f24c13a05e00a0d8a8aaba':
          data.NOTIFICATION_INQUIRY.INQUIRY_ACT_BY_OPERATION_ID_KO,
        '78c1f2769c8e4ba084a62e778a143d67':
          data.NOTIFICATION_INQUIRY.INQUIRY_ACT_BY_OPERATION_ID_NOT_FOUND,
      };

      const response =
        MAP_OPERATION_ID_TO_ACT_ERROR_RESPONSE[operationId as string] ??
      ...
      const MAP_OPERATION_ID_TO_AOR_ERROR_RESPONSE: { [key: string]: OperationAorResponse } = {
        '67d8a1f225f24c13a05e00a0d8a8aaba':
          data.NOTIFICATION_INQUIRY.INQUIRY_AOR_BY_OPERATION_ID_KO,
        '78c1f2769c8e4ba084a62e778a143d67':
          data.NOTIFICATION_INQUIRY.INQUIRY_AOR_BY_OPERATION_ID_NOT_FOUND,
      };

      const response =
        MAP_OPERATION_ID_TO_AOR_ERROR_RESPONSE[operationId as string] ??
  ...
  rest.post(`${API_BASE_URL}${PRACTICES_BY_INTERNAL_ID_PATH}`, (req, res, ctx) => {
    ...
    if (domain === 'act') {
      ...
      const response =
        MAP_INTERNAL_ID_TO_ACT_ERROR_RESPONSE[internalId as string] ??
      ...
      const response =
        MAP_INTERNAL_ID_TO_AOR_ERROR_RESPONSE[internalId as string] ??
