...
describe("api utils", () => {
    ...
    it("recursivePolling ok", async () => {
        const fakeApi = jest.fn().mockResolvedValue("Not done yet").mockResolvedValue("Not done yet").mockResolvedValue("Done");
    ...
    it("recursivePolling ko", async () => {
        const fakeApi = jest.fn().mockResolvedValue("Not done yet").mockResolvedValue("Not done yet").mockResolvedValue("Not done yet");
        const successVerifierFn = (res: string) => res === "Done";
        try {
            const res = await recursivePolling(fakeApi, successVerifierFn, 3, 100);
        } catch (error) {
