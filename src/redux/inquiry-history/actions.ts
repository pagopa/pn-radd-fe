...
const searchByInternalId = async (
  internalId: string,
  inquiryType: DocumentInquiryType,
  filterRequest: FilterRequest
) => {
  if (inquiryType === DocumentInquiryType.ACT) {
    return await NotificationInquiryApi.getActPracticesByInternalId(internalId, filterRequest).then(
      (res) => res
    );
  } else if (inquiryType === DocumentInquiryType.AOR) {
    return await NotificationInquiryApi.getAorPracticesByInternalId(internalId, filterRequest).then(
      (res) => res
    );
  }
...
export const searchInquiry = createAppAsyncThunk<OperationsResponse, InquirySearchForm>(
  'searchInquiry',
  async (params: InquirySearchForm, { rejectWithValue, dispatch }) => {
    try {
      dispatch(setLoadingStatus());
      const { inquiryType, iun, operationId, taxId, from, to, recipientType } = params;
