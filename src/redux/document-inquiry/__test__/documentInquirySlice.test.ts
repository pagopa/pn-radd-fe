...
import { DocumentInquiryFormAction, abortTransaction, completeTransaction, startInquiry, startTransaction, uploadFileAndStartTransaction } from '../actions';
...
describe("Document Inquiry state test", () => {
    ...
    it("Handle uploadFileAndStartTransaction action", async () => {
        const action = await store.dispatch(uploadFileAndStartTransaction({
            checksum: 'test',
            bundleId: 'test',
            zip: new Blob(),
    ...
    it("Handle startTransaction action", async () => {
        //Prepare state
        store.dispatch(setInquiryFormData({
            recipientTaxId: "test", 
            recipientType: 'PF', 
            qrCode: 'test', 
            delegateTaxId: '' 
        }));
        store.dispatch(setInquiryFileData({
            checksum: 'test',
            fileKey: 'test',
            versionToken: 'test'
        }));
        store.dispatch(setTransactionData({
            operationId: 'test', 
            urlList: []
        }));
    ...
    it("Handle completeTransaction action", async () => {
        ...
        const action = await store.dispatch(completeTransaction({inquiryType: DocumentInquiryType.AOR}));
